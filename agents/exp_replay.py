...
class NaiveRehearsal(NormalNN):
    ...
    def learn_stream(self, train_loader, new_task_next=True):
        ...
        for i, ((train_input, train_target),(replay_input, replay_target)) in enumerate(zip(train_loader, replay_loader)):
            ...
            # updating accuracy
            acc.update(accuracy(train_output, train_target, self.config["acc_topk"])[0], train_input.size(0))
            losses.update(train_loss, train_input.size(0))
            acc.update(accuracy(replay_output, replay_target, self.config["acc_topk"])[0], replay_input.size(0))
