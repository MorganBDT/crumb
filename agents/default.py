...
class NormalNN(nn.Module):
    ...
    def create_model(self):
        ...
        if cfg['model_weights'] is not None:
            ...
            cfg_load["n_class"] = cfg["pretrained_dataset_no_of_classes"]
            model = models.__dict__[cfg['model_type']].__dict__[cfg['model_name']](cfg_load)
            print('=> Load model weights: ' + cfg['model_weights'])
            # loading weights to CPU
            model_state = torch.load(cfg['model_weights'] + '.pth', map_location=torch.device("cuda"))
        ...
        if not cfg['continuing']:
            model.model_layers.classifier[1] = nn.Conv2d(512, cfg['n_class'], (3, 3), stride=(1, 1), padding=(1, 1))
    ...
    def validation(self, dataloader, metric_topk=(1,)):

        accs_avg = {}
        for k in metric_topk:
            accs_avg[k] = AverageMeter()
        batch_timer = Timer()
        batch_timer.tic()
        ...
        for i, (inputs, target) in enumerate(dataloader):
            ...
            accs = accuracy(output, target, metric_topk)

            for k_ind, k in enumerate(metric_topk):
                accs_avg[k].update(accs[k_ind], inputs.size(0))
        ...
        accs_avg = [acc.avg for acc in list(accs_avg.values())]

        total_time = batch_timer.toc()
        return accs_avg, total_time
    ...
    def learn_stream(self, train_loader, new_task_next=True):
        ...
        for i, (inputs, target) in enumerate(train_loader):
            ...
            acc.update(accuracy(output, target, self.config["acc_topk"])[0], inputs.size(0))
