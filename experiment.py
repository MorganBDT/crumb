...
def run(args, run, task_range=None):
    ...
    elif args.dataset in ["toybox", "ilab2mlight", "cifar100", "imagenet", "imagenet900", "ilab2mlight+core50"]:
        # image transformations
    ...
    all_accs = train(agent, composed, args, run, tasks, active_out_nodes, test_data, val_data, task_range=task_range)
...
def train(agent, transforms, args, run, tasks, active_out_nodes, test_data, val_data, task_range=None):
    ...
    for task in task_range:
        ...
        for epoch in range(n_epoch):
            ...
            if args.validate:
                val_accs, val_time = agent.validation(val_loader, args.acc_topk)
                print(' * Val Acc: {acc:.3f}, Time: {time:.2f}'.format(acc=val_accs[0], time=val_time))
                all_accs["direct"]["val_all"]["all_epochs"][task].append(val_accs)

            test_accs, test_time = agent.validation(test_loader, args.acc_topk)
            print(' * Test Acc: {acc:.3f}, Time: {time:.2f}'.format(acc=test_accs[0], time=test_time))
            all_accs["direct"]["test_all"]["all_epochs"][task].append(test_accs)
            ...
            if task == 0:
                test_accs_1st, test_time_1st = test_accs, test_time
                test_accs_current_task, test_time_current = test_accs, test_time
            else:
                test_accs_1st, test_time_1st = agent.validation(test_loader_1st, args.acc_topk)
                test_accs_current_task, test_time_current = agent.validation(test_loader_current_task, args.acc_topk)

            all_accs["direct"]["test_1st"]["all_epochs"][task].append(test_accs_1st)
            print(' * Test Acc (1st task): {acc:.3f}, Time: {time:.2f}'.format(acc=test_accs_1st[0], time=test_time_1st))
            all_accs["direct"]["test_current_task"]["all_epochs"][task].append(test_accs_current_task)
            print(' * Test Acc (current task): {acc:.3f}, Time: {time:.2f}'.format(acc=test_accs_current_task[0], time=test_time_current))
            ...
            if args.save_model or args.save_model_every_epoch:
                ...
                if args.save_model_every_epoch:
                    model_save_path = os.path.join(get_out_path(args), args.agent_name + "_run" + str(run) + "_task" + str(task) + "_epoch" + str(epoch))
                else:
                    model_save_path = os.path.join(get_out_path(args), args.agent_name + "_run" + str(run))
                agent.save_model(model_save_path)

            # Has no effect if args.lr_decrement is 0
        ...
        if (args.keep_best_net_all_tasks or (args.keep_best_task1_net and task == 0)) and args.n_epoch_first_task > 1:
            # Reload state of network when it had highest top-1 test accuracy on first task (or change to e.g. epch[1]
            comp_top1_accs = [epch[0] for epch in all_accs["direct"]["test_all"]["all_epochs"][task]]
            ...
            reload_test_accs, test_time = agent.validation(test_loader, args.acc_topk)
            print(' * Test Acc (after reloading best model): {acc:.3f}, Time: {time:.2f}'.format(acc=reload_test_accs[0], time=test_time))
            assert abs(reload_test_accs[0] - max_top1_acc) < 0.001, "Test accuracy of reloaded model does not match original highest test accuracy. Is the model saving and loading its state correctly?"

            # Set the test/val accs to be stored for this task to those corresponding to the best-performing network
            test_accs = all_accs["direct"]["test_all"]["all_epochs"][task][max_top1_acc_ind]
            test_accs_1st = all_accs["direct"]["test_1st"]["all_epochs"][task][max_top1_acc_ind]
            test_accs_current_task = all_accs["direct"]["test_current_task"]["all_epochs"][task][max_top1_acc_ind]
            if args.validate:
                val_accs = all_accs["direct"]["val_all"]["all_epochs"][task][max_top1_acc_ind]
        ...
        all_accs["direct"]["test_all"]["best_epochs"].append(test_accs)
        all_accs["direct"]["test_1st"]["best_epochs"].append(test_accs_1st)
        all_accs["direct"]["test_current_task"]["best_epochs"].append(test_accs_current_task)
        if val_data is not None:
            all_accs["direct"]["val_all"]["best_epochs"].append(val_accs)

    if args.save_model or args.save_model_every_epoch:
        agent.save_model(os.path.join(get_out_path(args), args.agent_name + "_run" + str(run)))
...
def get_args(argv):
    ...
    parser.add_argument('--model_type', type=str, default='squeezenet', help="The type (mlp|lenet|vgg|squeezenet) of backbone network")
    ...
    parser.add_argument('--output_dir', default='outputs', help="Where to store accuracy table")
