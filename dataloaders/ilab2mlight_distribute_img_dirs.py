...
# Change string class labels to class numbers
img_df["class_str"] = img_df.loc[:, "class"]
ilab2mlight_class_inds = pd.read_csv('ilab2mlight_classes.csv', index_col=0, usecols=["class", "index"], squeeze=True).to_dict()
img_df["class"] = img_df.apply(lambda row: ilab2mlight_class_inds[row["class_str"]], axis=1)
