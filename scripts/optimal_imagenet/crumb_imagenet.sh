...
python -u experiment_aug.py --scenario class_iid --freeze_feature_extract --acc_topk 1 5 --save_model_every_epoch --keep_best_task1_net --best_net_direct --specific_runs $RUN --n_epoch_first_task 15 --n_epoch 1 --replay_times 1 --replay_coef 5 --n_memblocks "$AUGMEM_SLOTS" --memblock_length "$AUGMEM_FEAT" --pretrained_dataset_no_of_classes "$PRETRAIN_N_CLASSES" --model_type squeezenet --model_name SqueezeNet --pretrained --agent_type crumb --agent_name Crumb --momentum 0.9 --weight_decay 0.0001 --batch_size 128 --n_workers 8 --pretrained_weights --model_weights "$weights_path" --memory_weights "$weights_path" --lr "$LR" --memory_size "$MEMORY_SIZE" --gpuid "$GPU" --dataset "$DATASET" --dataroot "$DATAROOT"  --output_dir "$OUTDIR" | tee "$OUTDIR"/class_iid/Crumb_SqueezeNet/runs-"$RUN"/log.log