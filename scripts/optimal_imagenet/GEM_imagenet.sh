...
# Usage: ./scripts/optimal_imagenet/GEM_imagenet.sh 1 0.001 10000 imagenet "/path/to/dataset" output_folder
...
OUTDIR=${6:-"${DATASET}_GEM_outputs_nmi_run${RUN}"}

mkdir -p "$OUTDIR"/class_iid/GEM_SqueezeNet/

python -u experiment.py --scenario class_iid --acc_topk 1 5 --keep_best_task1_net --pretrained --specific_runs $RUN --n_epoch_first_task 20 --n_epoch 1 --lr 0.001 --reg_coef 1 --model_type squeezenet --model_name SqueezeNet --agent_type exp_replay --agent_name GEM --momentum 0.9 --weight_decay 0.0001 --batch_size 128 --n_workers 8 --memory_size "$MEMORY_SIZE" --gpuid "$GPU" --dataset "$DATASET" --dataroot "$DATAROOT"  --output_dir "$OUTDIR" | tee "$OUTDIR"/class_iid/GEM_SqueezeNet/log.log