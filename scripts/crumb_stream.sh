...
if [ "$DATASET" = "core50+ilab2mlight" ]; then
  mkdir -p "$OUTDIR"/class_iid_transfer/Crumb_SqueezeNet/
fi
if [ "$DATASET" = "core50" ] || [ "$DATASET" = "toybox" ] || [ "$DATASET" = "ilab2mlight" ] || [ "$DATASET" = "core50+ilab2mlight" ]; then
  ...
  if [ "$DATASET" = "core50+ilab2mlight" ]; then
    mkdir -p "$OUTDIR"/class_instance_transfer/Crumb_SqueezeNet/
  fi
fi
...
if [ "$DATASET" = "core50+ilab2mlight" ]; then
  python -u experiment_aug.py --scenario class_iid_transfer      --save_model --n_runs "$N_RUNS" --n_epoch_first_task 10 --n_epoch 1 --replay_times 1 --replay_coef 5 --n_memblocks "$N_MEMBLOCKS" --memblock_length "$MEMBLOCK_LENGTH" --freeze_feature_extract --model_type squeezenet --model_name SqueezeNet --pretrained --agent_type crumb --agent_name Crumb --momentum 0.9 --weight_decay 0.0001 --batch_size "$BATCH_SIZE" --n_workers 8 --pretrained_weights --model_weights "$weights_path" --memory_weights "$weights_path" --pretrained_dataset_no_of_classes "$PRETRAIN_N_CLASSES" --lr "$LR" --memory_size "$MEMORY_SIZE" --gpuid "$GPU" --dataset "$DATASET" --dataroot "$DATAROOT"  --output_dir "$OUTDIR" | tee "$OUTDIR"/class_iid_transfer/Crumb_SqueezeNet/log.log
  python -u experiment_aug.py --scenario class_instance_transfer --save_model --n_runs "$N_RUNS" --n_epoch_first_task 10 --n_epoch 1 --replay_times 1 --replay_coef 5 --n_memblocks "$N_MEMBLOCKS" --memblock_length "$MEMBLOCK_LENGTH" --freeze_feature_extract --model_type squeezenet --model_name SqueezeNet --pretrained --agent_type crumb --agent_name Crumb --momentum 0.9 --weight_decay 0.0001 --batch_size "$BATCH_SIZE" --n_workers 8 --pretrained_weights --model_weights "$weights_path" --memory_weights "$weights_path" --pretrained_dataset_no_of_classes "$PRETRAIN_N_CLASSES" --lr "$LR" --memory_size "$MEMORY_SIZE" --gpuid "$GPU" --dataset "$DATASET" --dataroot "$DATAROOT"  --output_dir "$OUTDIR" | tee "$OUTDIR"/class_instance_transfer/Crumb_SqueezeNet/log.log
fi