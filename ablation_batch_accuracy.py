...
def make_visualizations(agent, transforms, args, run, tasks, active_out_nodes, test_data):
    ...
    test_accs_mem, test_accs_direct, all_accs_mem, all_accs_dir, test_time, all_targets = agent.validation(test_loader, args.acc_topk, get_all_accs=True, ignore_partial_batches=True)
    ...
    num_batches = len(all_accs_dir)

    df = pd.DataFrame({
        "ablation_name": [args.output_dir] * num_batches,
        "run": [run] * num_batches,
        "batch_order_hash": hash(tuple(all_targets)),
        "batch": list(range(num_batches)),
        "accuracy": [top1_top5_accs[0] for top1_top5_accs in all_accs_dir],
    })
    total_path = get_out_path(args)
    df.to_csv(os.path.join(total_path, "batchwise_accs_run" + str(run) + ".csv"))
