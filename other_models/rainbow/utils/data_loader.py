...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.dataset == "core50":
            ...
            img_path = os.path.join(dataroot, img_name)
        if self.dataset == "ilab2mlight+core50":
            dataroot = '/media/data/Datasets'
            img_path = os.path.join(dataroot, img_name)
        if self.dataset == "icubworldtransf":
...
def get_train_datalist(args, cur_iter: int) -> List:
    ...
    if args.dataset == "toybox":
        datalist, _ = toybox(args.scenario, args.run) #cifar100('class_iid',0)
    if args.dataset == "ilab2mlight":
        datalist, _ = ilab2mlight(args.scenario, args.run)
    if args.dataset == "core50":
        datalist, _ = core50(args.scenario, args.run)
    if args.dataset == "ilab2mlight+core50":
        datalist, _ = ilab2mlight_plus_core50(args.scenario, args.run)
    if args.dataset == "icubworldtransf":
        datalist, _ = icubworldtransf(args.scenario, args.run)
    if args.dataset == "cifar100":
        datalist, _ = cifar100(args.scenario, args.run)
    #print("datalist")
    #print(type(datalist))
...
def get_test_datalist(args, exp_name: str, cur_iter: int) -> List:
    ...
    if args.dataset == "toybox":
        datalist, test_datalist = toybox(args.scenario, 0) #here 0 stands for run number which is as of now 0th one
    if args.dataset == "ilab2mlight":
        datalist, test_datalist = ilab2mlight(args.scenario, 0)
    if args.dataset == "core50":
        datalist, test_datalist = core50(args.scenario, 0)
    if args.dataset == "ilab2mlight+core50":
        datalist, test_datalist = ilab2mlight_plus_core50(args.scenario, 0)
    if args.dataset == "icubworldtransf":
        datalist, test_datalist = icubworldtransf(args.scenario, 0)
    if args.dataset == "cifar100":
        datalist, test_datalist = cifar100(args.scenario, 0)
...
def get_statistics(dataset: str):
    ...
    mean = {
        ...
        "ilab2mlight": (0.485, 0.456, 0.406),
        "core50": (0.5071,0.4866,0.4409),
        "ilab2mlight+core50": (0.485, 0.456, 0.406),
        "icubworldtransf": (0.485, 0.456, 0.406),
    }
    ...
    std = {
        ...
        "ilab2mlight": (0.229, 0.224, 0.225),
        "core50": (0.2673,0.2564,0.2762),
        "ilab2mlight+core50": (0.229, 0.224, 0.225),
        "icubworldtransf": (0.229, 0.224, 0.225),
    }
...
def ilab2mlight(paradigm, run):
    batch_num = 7  # Number of tasks
...
def ilab2mlight_plus_core50(paradigm, run):
    batch_num = 12  # Number of tasks
    classes_per_task = 2
    rootdir = './../../dataloaders/ilab2mlight+core50_task_filelists/'
    rain_train = [[]]*batch_num
    rain_test = [[]]*batch_num
    train_data = []
    train_labels = []
    train_groups = [[]]*batch_num
    for b in range(batch_num):
        with open(rootdir + paradigm + '/run' + str(run) + '/stream/train_task_' + str(b).zfill(2) + '_filelist.txt',
                  'r') as f:
            # print("opened successfully")
            for i, line in enumerate(f):
                if line.strip():
    ...
    test_data = []
    test_labels = []
    test_groups = [[]]*batch_num

    with open(rootdir + paradigm + '/run' + str(run) + '/stream/test_filelist.txt', 'r') as f:
        for i, line in enumerate(f):
            if line.strip():
                path, label = line.split()
                gp = int(int(label) // classes_per_task)
                # print("label, gp")
                # print(label, gp)
                test_groups[gp].append((path, int(label)))
                test_data.append(path)
...
def icubworldtransf(paradigm, run):
    batch_num = 10  # Number of tasks
    classes_per_task = 2
