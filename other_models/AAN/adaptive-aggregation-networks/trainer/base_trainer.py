...
class BaseTrainer(object):
    ...
    def set_save_path(self):
        ...
        self.save_path = self.log_dir + self.args.dataset + \
            ...
            '_run' + str(self.args.runs) + \
            '_scenario_' + str(self.args.scenario) + \
...
#             Y_valid_total = np.array(self.testset.targets)
            ...
            for task in range(int(self.args.num_classes/self.args.nb_cl)):    
                train_data = datasets.Generic_Dataset(
                                dataroot=self.args.dataroot, dataset=self.args.dataset, filelist_root=self.args.filelist_root, scenario=self.args.scenario,
                                offline=self.args.offline, run=self.args.runs, batch=task, transform=self.transform_train)

                X_train_total = X_train_total + train_data.data
                Y_train_total = Y_train_total + train_data.labels
                
            # get test data
            test_data = datasets.Generic_Dataset(
                        dataroot=self.args.dataroot, dataset=self.args.dataset, filelist_root=self.args.filelist_root, scenario=self.args.scenario, offline=self.args.offline, run=self.args.runs, train=False, transform=self.transform_test)
    ...
    def update_train_and_valid_loader(self, X_train, map_Y_train, X_valid_cumul, map_Y_valid_cumul, \
        ...
        if self.args.dataset in ["core50", "toybox", "ilab2mlight", "cifar100", "imagenet", "imagenet900", "ilab2mlight+core50", "icubworldtransf"]:
            ...
            if self.args.scenario == 'class_iid':
