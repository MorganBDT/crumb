...
from imagenet_base_initialization_random_remind import *
from CRUMB_data_loaders import get_data_loader
import utils
...
def update_accuracies(args, curr_max_class, remind, pq, accuracies):
    base_test_loader = get_data_loader('val', args=args, min_class=args.min_class,
...
#         args.base_init_classes, curr_max_class - 1, non_base_top1, non_base_top5))
    ...
    seen_classes_test_loader = get_data_loader('val', args=args, min_class=args.min_class, max_class=curr_max_class)
...
#     accuracies['non_base_classes_top5'].append(float(non_base_top5))
    ...
    suffix = '_' + args.expt_name + '_task_' + str(args.task)
...
def streaming(args, remind):
    ...
    if args.resume_full_path is not None:
        ...
        test_loader = get_data_loader('val', min_class=args.min_class, max_class=args.streaming_min_class, args=args)
        ...
        pq, latent_dict, rehearsal_ixs, class_id_to_item_ix_dict = fit_pq(feat_data, label_data, item_ix_data,
                                                                          ...
                                                                          args.codebook_size, counter=counter, run=args.run)
        initial_test_loader = get_data_loader('val', min_class=args.min_class, max_class=args.base_init_classes, args=args)
    ...
    for class_ix in range(args.streaming_min_class, args.streaming_max_class, args.class_increment):
        ...
        train_loader_curr = get_data_loader('train', min_class=class_ix, max_class=max_class, args=args, return_item_ix=True)
        ...
        # perform inference
        test_loader = get_data_loader('val', min_class=args.min_class, max_class=max_class, args=args)
        _, probas, y_test = remind.predict(test_loader, pq)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--run', type=int, default=0)  # which run to do
    parser.add_argument('--dataset', type=str, required=True)  # name of the dataset (e.g. core50, ilab2mlight, toybox, cifar100)
    ...
    print("Total running time (seconds): " + str(time.time() - start_time))
