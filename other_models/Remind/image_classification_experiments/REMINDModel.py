...
import pickle
import utils
from retrieve_any_layer import ModelWrapper, ModelWrapperSqueezeNetFeatExtractor
...
class REMINDModel(object):
    ...
    def __init__(self, num_classes, classifier_G='ResNet18ClassifyAfterLayer4_1',
        ...
        if "SqueezeNet" in classifier_G:
            core_model = models.squeezenet1_0(pretrained=False)  # pretrained or not here shouldn't matter, these weights will be overwritten when pretrained model is loaded
            core_model.classifier[1] = nn.Conv2d(512, num_classes, (3, 3), stride=(1, 1), padding=(1, 1))  # random weights that will be overwritten when pretrained model is loaded
            ...
            self.classifier_F = SqueezeNetStartAfterLayer12(num_classes=num_classes, core_model=core_model)
    ...
    def fit_incremental_batch(self, curr_loader, latent_dict, pq, rehearsal_ixs=None, class_id_to_item_ix_dict=None,
        ...
        for batch_images, batch_labels, batch_item_ixs in curr_loader:
            ...
            codes = pq.compute_codes(np.ascontiguousarray(data_batch))
            ...
            for x, y, item_ix in zip(codes, batch_labels, batch_item_ixs):
                ...
                if self.use_mixup:
                    ...
                    for ii, v in enumerate(ixs):
                        try:
                            ...
                            print("ii: " + str(ii))
                            print("v: " + str(v))
                            print("Length of latent_dict: " + str(len(latent_dict)))
                            print("Latent dict keys: ")
                ...
                unique_ix = max(rehearsal_ixs) + int(item_ix.numpy())
                rehearsal_ixs.append(unique_ix)
                latent_dict[unique_ix] = [x, y.numpy()]
                class_id_to_item_ix_dict[int(y.numpy())].append(unique_ix)
                ...
                if self.max_buffer_size is not None and counter.count >= self.max_buffer_size:
                    ...
                    # print("Removing ix: " + str(rand_item_ix))
                    if rand_item_ix in latent_dict:
                        latent_dict.pop(rand_item_ix)
                    # max_class_list.remove(rand_item_ix)
                    max_class_list = remove_all(max_class_list, rand_item_ix)
                    # rehearsal_ixs.remove(rand_item_ix)
                    rehearsal_ixs = remove_all(rehearsal_ixs, rand_item_ix)
                    #class_id_to_item_ix_dict[max_key] = remove_all(class_id_to_item_ix_dict[max_key], rand_item_ix)
                else:
                    counter.update()
