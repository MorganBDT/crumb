...
def fit_pq(feats_base_init, labels_base_init, item_ix_base_init, num_channels, spatial_feat_dim, num_codebooks,
           codebook_size, batch_size=128, counter=utils.Counter(), run=0):
    """
    ...
    print('\nTraining Product Quantizer (by loading in pre-determined matrices)')
    start = time.time()
    nbits = int(np.log2(codebook_size))
    pq = faiss.ProductQuantizer(num_channels, num_codebooks, nbits)

    mem = np.load(os.path.join("starting_random_basis_matrices", "CRUMB_run" + str(run) + "_mem.npy"))
    assert pq.ksub == mem.shape[0], "Number of rows " + str(mem.shape[0]) + " in CRUMB basis matrix should match number of centroids " + str(pq.ksub)
    assert pq.dsub == mem.shape[1], "Number of columns " + str(mem.shape[1]) + " in CRUMB basis matrix should match number of centroids " + str(pq.dsub)

    centroids = np.repeat(mem.reshape((1, mem.shape[0], mem.shape[1])), num_codebooks, axis=0)
    print("shape of randomly generated centroids: " + str(centroids.shape))

    # pq.train(train_data_base_init)

    print("Copying randomly generated centroids into product quantizer...")
    faiss.copy_array_to_vector(centroids.ravel(), pq.centroids)
    print("PQ setup completed in {} secs".format(time.time() - start))
